...
const GAME_TYPES = [
  ...
  { id:"math",   icon:"➕", name:"Matematik Oyunu",  desc:"Hızlı hesap yarışması" },
...
export default function ProjectSelectScreen() {
  const { currentChild, setProjectType, setProjectFriend, setIsProjectHost, setScreen } = useApp()
  const [showGames, setShowGames] = useState(false)
  const [selectedGame, setSelectedGame] = useState(null)

  // Arkadaş bilgisini sessionStorage'dan oku
  const friend = (() => {
    try { return JSON.parse(sessionStorage.getItem('projectFriend')) } catch { return null }
  })()
        ...
        <div style={{ width:'100%', maxWidth:380 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            <button onClick={() => startSolo(selectedGame)}
              ...
              <div style={{ color:'rgba(255,255,255,.5)', fontSize:12, marginTop:3 }}>Kendi skorunu kır</div>
            </button>
            ...
            {friend ? (
              ...
              <div style={{ background:'rgba(255,255,255,.04)', border:'1.5px dashed rgba(255,255,255,.1)', borderRadius:16, padding:20 }}>
                <div style={{ fontSize:28, marginBottom:8 }}>👥</div>
                <div style={{ color:'rgba(255,255,255,.3)', fontSize:15, fontWeight:800 }}>Arkadaşınla Oyna</div>
                <div style={{ color:'rgba(255,255,255,.2)', fontSize:12, marginTop:3 }}>Önce arkadaşlar ekranından arkadaş seç</div>
              </div>
            )}
          </div>
        ...
        {!showGames ? (
          <>
            <div style={{ textAlign:'center', marginBottom:28 }}>
              ...
              <div style={{ color:'white', fontSize:20, fontWeight:900 }}>Ne yapmak istiyorsun? 🚀</div>
