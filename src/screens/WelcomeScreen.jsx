...
export default function WelcomeScreen({ onParent, onChild }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(160deg,#0f0a05 0%,#1C1410 60%,#0f0a05 100%)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'40px 24px', fontFamily:'Nunito,sans-serif', overflow:'hidden' }}>
      ...
      {/* Logo + Papağan */}
      <div style={{ opacity:show?1:0, transform:show?'scale(1)':'scale(0.85)', transition:'all 0.6s cubic-bezier(0.34,1.56,0.64,1)', display:'flex', flexDirection:'column', alignItems:'center', gap:4, marginBottom:12 }}>
      ...
      {/* Özellikler */}
      <div style={{ opacity:show?1:0, transition:'all 0.5s ease 0.3s', display:'flex', gap:16, marginBottom:36 }}>
      ...
      <div style={{ opacity:show?1:0, transform:show?'translateY(0)':'translateY(20px)', transition:'all 0.5s ease 0.4s', width:'100%', maxWidth:340, display:'flex', flexDirection:'column', gap:12 }}>
        ...
        {/* Veli */}
        <button onClick={onParent} style={{ width:'100%', padding:'18px', borderRadius:18, border:'none', background:'#F59E0B', color:'#0f0a05', fontWeight:900, fontSize:16, cursor:'pointer', fontFamily:'Nunito,sans-serif', display:'flex', alignItems:'center', justifyContent:'center', gap:10 }}>
          <span>👨‍👩‍👧</span>
          <span>Ben Veliyim</span>
        </button>

        {/* Çocuk */}
        <button onClick={onChild} style={{ width:'100%', padding:'18px', borderRadius:18, border:'2px solid rgba(245,158,11,.3)', background:'rgba(245,158,11,.08)', color:'#F59E0B', fontWeight:900, fontSize:16, cursor:'pointer', fontFamily:'Nunito,sans-serif', display:'flex', alignItems:'center', justifyContent:'center', gap:10 }}>
          <span>🧒</span>
          <span>Ben Çocuğum</span>
        </button>

        <div style={{ color:'rgba(255,255,255,.2)', fontSize:11, textAlign:'center', marginTop:4 }}>
          Çocuklar veli izniyle giriş yapar
        </div>
      </div>
    </div>
  )
