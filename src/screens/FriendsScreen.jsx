...
export default function FriendsScreen() {
  ...
  async function acceptFriend(friendship) {
    ...
    setPending(p => p.filter(f => f.id !== friendship.id)); loadFriends()
  }
  ...
  async function addFriend() {
    ...
    const { error } = await sb.from('friendships').insert({
      child_id_1: currentChild.id, child_id_2: found.id,
      status: 'pending', invited_by: currentChild.id, invite_code: inviteCode.toUpperCase()
    })
  ...
  function goToProject(friendData) {
    // sessionStorage'a yaz — ProjectSelectScreen okuyacak
