...
export default function FriendsScreen() {
  ...
  async function loadFriends() {
    ...
    // Her arkadaşlık için karşı tarafın ID'sini bul, sonra bilgilerini çek
            ...
            {pending.map(f => (
              <div key={f.id} style={{ background:'rgba(251,191,36,.1)', border:'1.5px solid rgba(251,191,36,.3)', borderRadius:16, padding:'14px 16px', marginBottom:10, display:'flex', alignItems:'center', gap:12 }}>
                <div style={{ width:44, height:44, borderRadius:'50%', overflow:'hidden', background:'rgba(255,255,255,.1)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22, flexShrink:0 }}>
                  {f.senderData?.avatar_photo
                    ? <img src={f.senderData.avatar_photo} style={{width:'100%',height:'100%',objectFit:'cover'}}/>
                    : f.senderData?.avatar_emoji || '👤'}
                </div>
                <div style={{ flex:1 }}>
        ...
        {friends.length === 0 ? (
          ...
          <div key={f.id} style={{ background:'rgba(255,255,255,.06)', border:'1.5px solid rgba(255,255,255,.1)', borderRadius:16, padding:'14px 16px', marginBottom:10, display:'flex', alignItems:'center', gap:12 }}>
            <div style={{ width:48, height:48, borderRadius:'50%', overflow:'hidden', background:'rgba(255,255,255,.1)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:24, flexShrink:0 }}>
              {f.friendData?.avatar_photo
                ? <img src={f.friendData.avatar_photo} style={{width:'100%',height:'100%',objectFit:'cover'}}/>
                : f.friendData?.avatar_emoji || '👤'}
            </div>
            <div style={{ flex:1 }}>
