...
export default function HistoryPanel({ child, onClose, onLoadSession }) {
  ...
  async function loadAll() {
    ...
    if (sess?.length) {
      ...
      const msgCount = {}
      msgs?.forEach(m => {
        if (!firstMsg[m.session_id] && m.role === 'user') firstMsg[m.session_id] = m.content
    ...
    const { data: ast } = await sb.from('chat_assets')
                  ...
                  <div style={{ display:'flex', alignItems:'flex-start', gap:8 }}>
                    <div onClick={() => onLoadSession(s.id)} style={{ flex:1, cursor:'pointer' }}>
                      ...
                      <div style={{ color:'rgba(255,255,255,.3)', fontSize:10, marginTop:3 }}>
                        {new Date(s.started_at).toLocaleDateString('tr-TR', {day:'numeric', month:'short'})} • {s.message_count} mesaj
                      </div>
                    </div>
                    <div style={{ display:'flex', gap:4, flexShrink:0 }}>
                      <button onClick={() => pinSession(s.id, s.pinned)} style={{ width:24, height:24, borderRadius:6, background:s.pinned?'rgba(13,155,126,.3)':'rgba(255,255,255,.08)', border:s.pinned?'1px solid #4ade80':'none', cursor:'pointer', color:s.pinned?'#4ade80':'rgba(255,255,255,.5)', fontSize:11 }}>📌</button>
                      <button onClick={() => deleteSession(s.id)} style={{ width:24, height:24, borderRadius:6, background:'rgba(239,68,68,.15)', border:'none', cursor:'pointer', color:'#fca5a5', fontSize:11 }}>🗑️</button>
                    </div>
                  </div>
                </div>
              ))
                ...
                {assets.map(a => (
                  <div key={a.id} style={{ background:'rgba(255,255,255,.05)', borderRadius:10, padding:'10px 12px', marginBottom:6, display:'flex', alignItems:'center', gap:10 }}>
                    {a.type === 'image' && a.url
                      ...
                      : <div style={{ width:40, height:40, borderRadius:6, background:'rgba(255,255,255,.08)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, flexShrink:0 }}>{a.type==='presentation'?'📊':'📄'}</div>
                    }
